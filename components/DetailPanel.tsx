                ...
                : project.source === "1기신도시"
                  ...
                  : project.source === "모아타운"
                    ...
                    : project.source === "서울플랜+"
                      ...
                      : project.source === "지자체포털"
                        ? `${project.portal?.site ?? "자치구 정비사업 포털"}의 사업현황 목록. 정보몽땅(추진위·조합 등록)과 서울플랜+(정비계획 입안 이후)에는 없는 안전진단·기본계획 등 초기 단계 사업장을 보충한 것으로, 진행상태·면적·세대수는 구청 입력값이며 경계는 대표지번 필지`
                        : "인천광역시 도시 및 주거환경 정비사업 추진현황(공공데이터포털, 월간)"}{" "}
              기준.
              {project.source === "지자체포털" && project.portal?.url ? (
                <>
                  {" "}
                  <a href={project.portal.url} target="_blank" rel="noreferrer" className="underline hover:text-brand">
                    포털 사업현황 목록
                  </a>
                </>
              ) : null}
