...
- `DATA_GO_KR_KEY`(공공데이터포털 인증키, 국토부 건축HUB 건축물대장정보 서비스 활용신청 필요)는 자료 빌드에서 후기 단계 사업장의 준공을 건축물대장 총괄표제부 사용승인일로 확정하는 데 쓰인다. 없으면 이전 자료의 확인 결과를 유지한다. 주간 자동 갱신에도 같은 이름의 시크릿을 넣는다.
