...
try {
  ...
  if ((force || !sessionStorage.getItem("rm_splash_seen")) && !matchMedia("(prefers-reduced-motion: reduce)").matches) {
    ...
    // 앱 CSS 가 도착하기 전에 본문이 잠깐 보이는 것을 막는 바탕색 덮개(라이트 흰색·다크 --bg). 인트로(z 100) 아래, 본문 위.
    // layout.tsx 의 THEME_SCRIPT 가 먼저 돌아 html.dark 를 정해 두므로 그 값을 본다.
    // 인트로가 걷히기 시작하면(.splash-out) 덮개도 같이 사라져 페이드가 본문으로 이어진다
    var dark = document.documentElement.classList.contains("dark");
    var st = document.createElement("style");
    st.textContent =
      "html.hl-splash::before{content:'';position:fixed;inset:0;z-index:99;background:" + (dark ? "#0f1115" : "#fff") + "}" +
